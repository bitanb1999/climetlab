...
class Enum:
    ...
    def __init__(self, param):
        ...
        if self._param._defs.get("option"):
            self.values = []
            self.values.extend(o["fortran"] for o in self._param._defs.get("option"))
...
class Param:
    def __init__(self, defs):
        self._defs = defs
        self._type = None
        if self.type.values in [[False, True], [True, False]]:
...
class Klass:
    ...
    def action(self):
        ...
        return None if action is None or action[0] == action[0].upper() else action
            ...
            if self._defs.get("inherits"):
                for p in self._defs.get("inherits").split("/"):
                    try:
                        ...
                        print(f"Cannot find super class '{p}' for '{self.name}'", file=sys.stderr)
...
def produce_rst():
    print(
        f".. DO NOT EDIT - This page is automatically generated by {os.path.basename(sys.argv[0])}"
    )
    print()
    print("Plotting")
    print("========")
    ...
    for action, klasses in sorted(ACTIONS.items()):
        print()
        print(f".. _magics-{action}:")

        print()
        print(action)
        print("-" * len(action))
        print()
        print(f".. {[k.name for k in sorted(klasses)]}")
        print()
        documentation = [k.documentation for k in sorted(klasses)]
...
def produce_python():
    ...
    for action, klasses in sorted(ACTIONS.items()):
        ...
        for k in sorted(klasses):
            print(f"    # [{k.name}]", k.documentation)
            for p in k.parameters:

                c = "#" if p.name in seen else ""

                print("   ", f"{c}{p.name}: {p.python_type} = {p.python_default},")
                seen.add(p.name)
                            # print("       |", p.documentation)
                            # print("     - |", p.values)
                            # print("     - |", p.default)
                            # print("     -", p.documentation)
        print("):")
        print(f"    return macro.{action}(**_given_args(inspect.currentframe()))")
...
def produce_schemas(directory):
    ...
    for action, klasses in sorted(ACTIONS.items()):
        ...
        path = os.path.join(directory, f"{action}.json")
        with open(f"{path}.tmp", "w") as f:
